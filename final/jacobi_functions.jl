...
function jacobi_A_hessen(A::Array, l::Integer)  
    # Jacobi algorithm #1 for computing eigenvalues with transformation to Hessenberg form 
    n = size(A,1);
...
function jacobi_B_vect_hessen(A::Array, l::Integer)  
    # Jacobi algorithm #2 for computing eigenvalues with transformation to Hessenberg form
    A = HessenbergReduction(A)
    n = size(A,1);
    X = copy(A);
    H = eye(n);  # H is a eigenvectors matrix
...
function jacobi_B_hessen(A::Array, l::Integer)  
    # Jacobi algorithm #2 for computing only eigenvalues with transformation to Hessenberg form
    A = HessenbergReduction(A)
    n = size(A,1);
    X = copy(A);
    for iter = 1:l
